...
const SearchResults = (props) => {
     const { setCenter, results, dayOfTheWeek, center} = props;
     ...
     const handleDescriptionClick = (index) => {
          const newShowDescription = showDescription.map((_, i) => i === index ? !showDescription[index] : false);
     ...
     return (
          <>
            {results.length !== 0 ? results.map((result, index) => {
              ...
              return (
                <div 
                  className={`${finalCard} ${styles.card} ${showDescription[index] ? styles.cardSelected : ""}`} 
                  key={`resultCard-${index}`} onClick={() => 
                    {
                ...
                >
                  <div className={`ms-3 mt-3 mb-3 card-body ${styles.cardContent}`}>
                    ...
                    <div className={`mb-1 ${!result.streetAddress ? "d-none" : ""}`}>
                      <h6 className="col d-inline">
                        Address: &nbsp;
                      </h6>
                      <p className="col d-inline">
                        {result.streetAddress}, {result.city}, {result.state} {result.zipcode}, {result.country}
                      </p>
                    </div>
                      ...
                      <>
                        ...
                        <div>
                          {result.businessHours.map((businessHours, index) => {
                            const closingTime = !businessHours.close ? "Unavailable" : businessHours.close;
                            const openingTime = !businessHours.open ? "Unavailable" : businessHours.open;
                            const timeOpen = (!businessHours.close && !businessHours.open) ? "Closed" : `${openingTime} - ${closingTime}`;
                            return (
                              <p key={`day-${index}`} 
                                className={`mb-0 ${todayIs === businessHours.day ? 'fw-bold' : 'fw-light'}`}
                              >
